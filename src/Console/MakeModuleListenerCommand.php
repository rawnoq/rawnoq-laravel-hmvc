...
class MakeModuleListenerCommand extends ListenerMakeCommand
{
    ...
    protected function getPath($name)
    {
        if ($this->moduleName) {
            ...
            if (Str::startsWith($relative, 'Listeners'.DIRECTORY_SEPARATOR)) {
                $relative = Str::after($relative, 'Listeners'.DIRECTORY_SEPARATOR);
            }

            $primary = str_replace('/', DIRECTORY_SEPARATOR, $this->modulePrimaryDirectory($this->moduleName, 'listeners', 'Listeners'));
    ...
    protected function getOptions()
    {
        return array_merge(parent::getOptions(), [
            ['module', null, InputOption::VALUE_OPTIONAL, 'The module to create the listener in'],
        ]);
    }
}
