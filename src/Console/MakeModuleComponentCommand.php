...
class MakeModuleComponentCommand extends ComponentMakeCommand
{
    use ResolvesModules;
    ...
    {
        if ($this->moduleName) {
            return rtrim($rootNamespace, '\\').'\\View\\Components';
        }
    ...
    protected function getPath($name)
    {
        if ($this->moduleName) {
            ...
            if (Str::startsWith($relative, 'View'.DIRECTORY_SEPARATOR.'Components'.DIRECTORY_SEPARATOR)) {
                $relative = Str::after($relative, 'View'.DIRECTORY_SEPARATOR.'Components'.DIRECTORY_SEPARATOR);
            }

            $primary = str_replace('/', DIRECTORY_SEPARATOR, $this->modulePrimaryDirectory($this->moduleName, 'components', 'View/Components'));
    ...
    protected function getOptions()
    {
        return array_merge(parent::getOptions(), [
            ['module', null, InputOption::VALUE_OPTIONAL, 'The module to create the component in'],
        ]);
    }
}
