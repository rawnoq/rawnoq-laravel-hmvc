...
class MakeModuleJobMiddlewareCommand extends JobMiddlewareMakeCommand
{
    use ResolvesModules;
    ...
    {
        if ($this->moduleName) {
            return rtrim($rootNamespace, '\\').'\\Jobs\\Middleware';
        }
    ...
    protected function getPath($name)
    {
        if ($this->moduleName) {
            ...
            if (Str::startsWith($relative, 'Jobs'.DIRECTORY_SEPARATOR.'Middleware'.DIRECTORY_SEPARATOR)) {
                $relative = Str::after($relative, 'Jobs'.DIRECTORY_SEPARATOR.'Middleware'.DIRECTORY_SEPARATOR);
            }

            $primary = str_replace('/', DIRECTORY_SEPARATOR, $this->modulePrimaryDirectory($this->moduleName, 'job-middleware', 'Jobs/Middleware'));
    ...
    protected function getOptions()
    {
        return array_merge(parent::getOptions(), [
            ['module', null, InputOption::VALUE_OPTIONAL, 'The module to create the job middleware in'],
        ]);
    }
}
