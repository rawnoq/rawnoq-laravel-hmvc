...
class MakeModuleMailCommand extends MailMakeCommand
{
    ...
    protected function getPath($name)
    {
        if ($this->moduleName) {
            ...
            if (Str::startsWith($relative, 'Mail'.DIRECTORY_SEPARATOR)) {
                $relative = Str::after($relative, 'Mail'.DIRECTORY_SEPARATOR);
            }

            $primary = str_replace('/', DIRECTORY_SEPARATOR, $this->modulePrimaryDirectory($this->moduleName, 'mail', 'Mail'));
    ...
    protected function getOptions()
    {
        return array_merge(parent::getOptions(), [
            ['module', null, InputOption::VALUE_OPTIONAL, 'The module to create the mail in'],
        ]);
    }
}
